...
contract('EIP20', accounts => {
  ...
  it('transfers: should fail when trying to transfer 10001 to accounts[1] with accounts[0] having 10000', async () => {
    await assertRevert(
      HST.transfer.call(accounts[1], 10001, { from: accounts[0] })
    )
  })

  it('transfers: should handle zero-transfers normally', async () => {
    assert(
      await HST.transfer.call(accounts[1], 0, { from: accounts[0] }),
  ...
  it('approvals: msg.sender approves accounts[1] of 100 & withdraws 20 once.', async () => {
    ...
    await HST.transferFrom.call(accounts[0], accounts[2], 20, {
      from: accounts[1]
    })
  ...
  it('approvals: msg.sender approves accounts[1] of 100 & withdraws 50 & 60 (2nd tx should fail)', async () => {
    ...
    // onto next.
    await assertRevert(
      HST.transferFrom.call(accounts[0], accounts[2], 60, { from: accounts[1] })
    )
  })
  ...
  it('approvals: allow accounts[1] 100 to withdraw from accounts[0]. Withdraw 60 and then approve 0 & attempt transfer.', async () => {
    ...
    await assertRevert(
      HST.transferFrom.call(accounts[0], accounts[2], 10, { from: accounts[1] })
    )
  })
