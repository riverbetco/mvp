...
contract('EIP20Factory', accounts => {
  it('Verify a Human Standard Token once deployed using both verification functions.', async () => {
    ...
    const newTokenAddr = await factory.createEIP20.call(
      100000,
      'Simon Bucks',
      2,
      'SBX',
    ...
    await factory.createEIP20(100000, 'Simon Bucks', 2, 'SBX', {
      from: accounts[0]
    })
    const res = await factory.verifyEIP20.call(newTokenAddr, {
      from: accounts[0]
    })
